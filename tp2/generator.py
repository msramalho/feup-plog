...
def printConfiguration(data):
    ...
    print("\nScientific areas (%d):" % len(data["scientificAreas"]))
    print(pd.DataFrame(data["scientificAreas"], columns=[
          "id", "name"]).to_string(index=False))

    print("\nTeachers (%d):" % len(data["teachers"]))
    pdTeachers = pd.DataFrame(data["teachers"], columns=[
                              "id", "name", "type", "areas", "HS1", "HS2"])
...
def testConfiguration(data):
    ...
    pdTeachers = pd.DataFrame(data["teachers"], columns=["id", "name", "type", "areas", "HS1", "HS2"])
    pdSubjects = pd.DataFrame(data["subjects"], columns=[ "id", "name", "semester", "HT", "HP", "areas"])
