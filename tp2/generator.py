...
def printConfiguration(data):
    ...
    pdTeacherTypes = pd.DataFrame(data["teacherTypes"], columns=[
                                  "id", "name", "averageWeekHours"])
    ...
    print(pd.DataFrame(data["scientificFields"], columns=[
          "id", "name"]).to_string(index=False))

    print("\nTeachers (%d):" % len(data["teachers"]))
    pdTeachers = pd.DataFrame(data["teachers"], columns=[
                              "id", "name", "type", "fields", "HS1", "HS2"])
    print(pdTeachers.to_string(index=False))

    print("\nSubjects (%d):" % len(data["subjects"]))
    pdSubjects = pd.DataFrame(data["subjects"], columns=[
                              "id", "name", "semester", "HT", "HP", "durationT", "durationP" "fields"])
...
def testConfiguration(data):
    ...
    pdTeachers = pd.DataFrame(data["teachers"], columns=["id", "name", "type", "fields", "HS1", "HS2"])
    pdSubjects = pd.DataFrame(data["subjects"], columns=[ "id", "name", "semester", "HT", "HP", "fields"])
...
def convertToProlog(data, filename):
    ...
    for s in data["subjects"]:
        content += ("subject(%d, %d, %2d, %2d, %d, %d, %s). %% %s\n" % (s["id"], s["semester"], s["HT"], s["HP"], s["durationT"], s["durationP"], listToStr(s["fields"]), s["name"]))
    ...
    for t in data["teachers"]:
        content += ("\nteacher(%d, %d, %2d, %2d, %s). %% %s" % (t["id"], t["type"], t["HS1"], t["HS2"], listToStr(t["fields"]), t["name"]))
    return content
